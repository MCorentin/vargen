...
#' @title Create an EWAS catalog dataframe
#' @description Create an EWAS catalog dataframe by reading the file found in
#' the vargen directory as downloaded by the vargen_install function.
#' @return a an EWAS catalog dataframe containing the information
#' from the ewas catalog.
#'
#' @examples
#' ewas_cat <- create_ewas()
create_ewas <- function(){
  # 1. Find all the files that start with EWAS and let the user chose which file
  # they want to select.
  files <- list.files(pattern = "EWAS_Catalog")
  # 2. Get the input from the user about which file they wish to use.
  chosen_cat <- ""
  if(length(files) > 1) {
    print("Multiple EWAS Catalogs found")
    cnt <- 1
    for(file in files) {
      print(paste(cnt, file, sep = ": "))
      cnt <- cnt + 1
    }
    index <- as.numeric(readline(prompt = "Please select the number of the file desired as printed above: "))
    chosen_cat <- files[index]
  }
  # 3. Read the file into a data frame.
  ewas_cat <- read.table(gzfile(chosen_cat), sep = "\t", header = TRUE)
  # 4. Make the file into a GRange object
  return(ewas_cat)
}

#' @title List of EWAS traits
#' @description Creates a list of traits corresponding to the EWAS catalogs
#' based upon a given keyword or words from the user.  If the words are not
#' found then the function stops and provides a warning.
#' @param keywords: A single trait or vector of traits that is to be
#' searched through for other related terms.
#' @param ewas_cat: The dataframe containing the EWAS catalog.
#' @return vector of unique traits found for te given search parameter
#' including missing values.
#'
#' @examples
#' ewas_smoking_list <- list_ewas_traits("smoking", ewas_cat)
#' ewas_FJ_consumption_list <- list_ewas_traits(c("Fruit consumption", "Juice consumption"), ewas_cat)
list_ewas_traits <- function(keywords = "", ewas_cat) {
  if(missing(ewas_cat)){
    ewas_cat <- create_ewas()
    warning("ewas_cat not provided.", "Created one with create_ewas().")
  }
  
  traits <- c()
  # 1. Collapsing the values with a "|" for "OR" in grep search
  keys <- paste(keywords, collapse = "|")
  traits <- unique(ewas_cat[["Trait"]][grep(keys, ewas_cat[["Trait"]],
                                            ignore.case = TRUE)])
  if(length(traits) == 0) {
    print(paste0("No ewas traits found for '", keys, "'"))
  }
  
  return(unique(traits))
}

#' @title Get Single EWAS Trait Information
#' @description Gets the EWAS data associated with a certain trait or traits.
#' @param ewas_traits: A single trait or vector of traits that are found in the EWAS
#' catalog that are to be used restrict the data by. A trait must be given for this
#' function to run else an error will occur.
#' @param ewas_cat: The dataframe containing the EWAS catalog.
#' @return vector of unique traits found for te given search parameter
#' including missing values.
#'
#' @examples
#' ewas_smoking_traits <- get_ewas_trait_info("Smoking", ewas_cat)
#' ewas_FJ_consumption_traits <- get_ewas_trait_info(c("Fruit consumption", "Juice consumption"), ewas_cat)
get_ewas_trait_info <- function(ewas_traits, ewas_cat){
  if(missing(ewas_cat)){
    ewas_cat <- create_ewas()
    warning("ewas_cat not provided.", "Created one with create_ewas().")
  }
  ewas_variants <- subset(ewas_cat, ewas_cat[["Trait"]] %in% ewas_traits)
  ewas_variants <- ewas_variants[, c("PMID", "EFO", "Trait", "Chr", "Gene", "Tissue", "Pos")]
  
  return(ewas_variants)
}

#' @title Get an annotated EWAS dataframe
#' @description Links and annotates information from the GWAS, EWAS, and
#' GTEX datasets into one dataframe based upon their shared genes. The Files
#' are linked by first finding common tissue names between the GWAS and GTEx files
#' (involving the removal of non key words to improve matching such as 'and',
#' 'nor', 'normal', etc). It should be of note that Cancer and normal tissue samples
#' are grouped broadly together if they are of the same overarching tissue type,
#' say 'Lung'. The original GWAS tissue is kept in the final dataset for quality
#' assurances. Next the genes' are used to link the GTEx information directly with the
#' EWAS line by line since they are unique in each dataset. Following this the
...
get_ewas_anno <- function(ewas_cat, gtex_dir) {
  ...
  if(missing(gtex_dir)) {
    stop("Please specify the path to the GTEx directory (eg: 'GTEx_Analysis_v8_eQTL')")
  }
  # 1. Save a copy of the origional tissue labels.
  ewas_cat_copy <- ewas_cat
  
  # 2. Get the tissues and file paths for the gtex files.
  gtex_tissues <- list_gtex_tissues(gtex_dir)
  file_paths <- list.files(path = paste0(gtex_dir), full.names = TRUE)
  var_gene_pairs_paths <- file_paths[grep(".txt.gz", file_paths)]
  ...
  # broader catagory of tissues in the GTEX tissues dataset. This means inclusion of tissues
  # of a cancerious type are allows into the general catagory for matching.
  words <- paste0(c("cell", "cells" ,"cancer", "tumor", "tumors",
                    ",", "\\sand\\s", "\\sor\\s", "normal", "abnormal"), collapse = "|")
  ewas_cat[["Tissue"]] <- gsub(words, "", tolower(ewas_cat[["Tissue"]]))
  ewas_cat[["Tissue"]] <- gsub("\\s", "_", tolower(ewas_cat[["Tissue"]]))
  ...
  gtex_file <- c()
  for(file in na.omit(unique(ewas_cat_anno[["filepaths"]]))) {
    tissue_file <- read.table(gzfile(file), sep = "\t", header = TRUE)
    EWAS_tissue <- replicate(nrow(tissue_file), tissues[cnt])
    tissue_file$"GTEx_tissue_File" <- EWAS_tissue
    
    gtex_file <- rbind(gtex_file, tissue_file)
    cnt = cnt + 1
  }
  
  # Read in tissue file related to the found filepaths.
  gtex_file[["gene_id"]] <- gsub("\\..*", "", gtex_file[["gene_id"]])
  
  View(gtex_file)
  
  # Merge the datasets.
  colnames(gtex_file)[2] <- "Gene"
  gtex_file <- merge(ewas_cat_anno, gtex_file, by = c("Gene"), all.x = TRUE)
  
  # Get the gwas trait from the catalog
  gwas_cat <- create_gwas()
  gwas_traits <- cbind(gwas_cat$"DISEASE/TRAIT", gwas_cat$"SNP_GENE_IDS")
  colnames(gwas_traits) <- cbind("GWAS_trait", "gene_id")
  
  View(gtex_file)
  View(gwas_file)
  
  gtex_file <- merge(gtex_file, gwas_traits, by = c("gene_id"), all.x = TRUE)
  
  View("hit")
  
  # Reorganize the dataset to make a little more sense.
  gtex_file <- gtex_file[, c("GTEx_tissue_File", "gene_id", "Gene", "gene_chr", "gene_start",
                             "gene_end", "Tissue", "EWAS_Tissue", "EFO",
                             "GWAS_trait", "Trait", "Pos", "pval_true_df", "tss_distance",
                             "ref", "alt", "maf", "log2_aFC")]
  colnames(gtex_file) <- c("GTEx_tissue_File", "ensembl_gene_id", "hgnc_symbol", "chr", "gene_start",
                           "gene_end", "GTEx_tissue", "EWAS_tissue", "EFO",
...
vargen_install <- function(install_dir = "./", gtex_version = "v8", verbose = FALSE){
  ...
  # Downloading the EWAS MRC-IEU files.
